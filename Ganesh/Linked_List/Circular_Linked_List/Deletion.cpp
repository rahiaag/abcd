...
node *q = head;              //giving q as head so we can free the memory
head=head->next;
...
// Case 2 : Deleting the element in Between the Circular linked list
...
node *deleteByValue(node *head, int value)   //This Function Will Not Delete the Last Value
{
    ...
    if (q->data == value)
    { // we are checking because we are at last position so if we put any element which is not in linked list so it will delete the last element that's why we are checking for this
        p->next = q->next;
        free(q);
    }
    return head;
